...
typedef struct ownerQueue{
    int front, rear, total_income;
    Owner* ice_owner[MAX_DAY_SIZE+1];
}OwnerQueue;
...
int delivery_order(Customer* cus){
    ...
    for(i = cus->cup_size; i>0; i--){//아이스크림 하나씩 까보기
...
int prepare_order(Owner* ice_owner){
    ...
    printf("이번 고객님 요청사항 : 컵사이즈 %d번", cus->cup_size);
    for(i = 0; i<cus->cup_size; i++){
        int idx = cus->order[i]-1;
        int row, col;
        row = idx / COL;
        col = idx % COL;
        printf(", %s맛",icecream_tb[row][col]->name);
    }
    printf("입니다.");
    ...
    int choice;//for문 / 선택한 아이스크림 저장 변수

    for(i=0; i<cupsize; i++){
        printf("사장 : 몇 번 아이스크림을 쌓아야 하지?\n");
        scanf("%d", &choice);//아이스크림 받기
        push(choice-1, icecream_cup);//받은 값 아이스크림에 저장
    }
...
int get_order(Owner* ice_owner){
    ...
    for(i = 0; i < cus->cup_size; i++){
        ...
        cus->order[i] = ice_num+1;//i번째 아이스크림 저장
...
int is_empty_ownerQueue(OwnerQueue* oq){
    return oq->front == oq->rear;
}

void day_init(OwnerQueue *oq, int playday){
    memset(oq, 0, sizeof(OwnerQueue));
    for(int i = 0; i < playday; i++){
        oq->ice_owner[i] = (Owner*)malloc(sizeof(Owner));
        owner_init(oq->ice_owner[i]);
        oq->rear++;
    }
}

Owner* owner_dequeue(OwnerQueue* oq){
    if(is_empty_ownerQueue(oq)) return NULL;
    oq->front = (oq->front+1)%(MAX_DAY_SIZE+1);
    printf("%d일차 주문\n", oq->front);
    return oq->ice_owner[oq->front-1];
}
...
int main(){
    ...
    for(i = 0; i < playday; i++){
        oq.total_income += oq.ice_owner[i]->total_income;
    }

    printf("최종 수익: %d\n", oq.total_income);//최종수익 출력
